...
export const Wishlist = () => {
  ...
  useEffect(()=>{
    if(wishlistItemDeleteStatus==='fulfilled'){
        ...
        toast.error("Có lỗi khi xóa sản phẩm ra khỏi danh sách yêu thích, vui lòng thử lại sau")
    }
  ...
  useEffect(()=>{
    if(wishlistItemUpdateStatus==='fulfilled'){
      ...
      toast.error("Lỗi khi cập nhật danh mục mong muốn")
    }
  ...
  useEffect(()=>{
    if(wishlistFetchStatus==='rejected'){
      toast.error("Có lỗi khi tải danh sách yêu thích, vui lòng thử lại sau")
    }
  },[wishlistFetchStatus])
    ...
    <Stack justifyContent={'flex-start'} mt={is480?3:5} mb={'14rem'} alignItems={'center'}>
        ...
        <Stack width={is1130?"auto":'70rem'} rowGap={is480?2:4}>
            ...
            <Stack alignSelf={'flex-start'} flexDirection={'row'} columnGap={1} justifyContent={'center'} alignItems={'center'}>
                ...
                <Typography variant='h4' fontWeight={500}>Danh sách yêu thích của bạn</Typography>
            </Stack>
...

              {
                !wishlistFetchStatus==='pending' && wishlistItems?.length===0?(
                  ...
                  <Stack minHeight={'60vh'} width={is642?'auto':'40rem'} justifySelf={'center'}  alignSelf={'center'} justifyContent={'center'} alignItems={'center'}>
                    ...
                    <Typography variant='h6' fontWeight={300}>Bạn chưa có sản phẩm nào trong danh sách yêu thích</Typography>
                  </Stack>
                ):
                // wishlist grid
                      ...
                      <Stack component={is480?"":Paper} elevation={1} >
                        ...
                        <Stack paddingLeft={2} paddingRight={2} paddingBottom={2}>
                          ...
                          <Stack flexDirection={'row'} alignItems={'center'}>
                            <Typography variant='h6' fontWeight={400}>Ghi chú</Typography>
                            ...
                            editIndex===index?(
                              ...
                              <Stack rowGap={2}>
                                ...
                                <Stack flexDirection={'row'} alignSelf={'flex-end'} columnGap={1}>
                                    <Button onClick={()=>handleNoteUpdate(item._id)} size='small' variant='contained'>Cập nhật</Button>
                                    <Button onClick={()=>setEditIndex(-1)} size='small' variant='outlined' color='error'>Hủy</Button>
                                </Stack>
...

                          {
                            ...
                            <Button sx={{mt:4}} size='small' variant='outlined' component={Link} to={'/cart'}>Đã có trong giỏ hàng</Button>:<Button sx={{mt:4}} size='small' onClick={()=>handleAddToCart(item.product._id)} variant='outlined'>Thêm vào giỏ hàng</Button>
                          }
