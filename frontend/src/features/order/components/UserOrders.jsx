...
export const UserOrders = () => {
    ...
    useEffect(()=>{
        if(orderFetchStatus==='rejected'){
            toast.error("Có lỗi khi tải đơn hàng, vui lòng thử lại sau")
        }
    },[orderFetchStatus])
            ...
            <Stack width={is1200?"auto":"60rem"} p={is480?2:4} mb={'5rem'}>
                ...
                <Stack flexDirection={'row'} columnGap={2} >
                    ...
                    <Stack rowGap={1} >
                        ...
                        <Typography sx={{wordWrap:"break-word"}} color={'text.secondary'}>Kiểm tra trạng thái của các đơn hàng gần đây, quản lý việc trả lại và khám phá các sản phẩm tương tự.</Typography>
                    </Stack>
                ...
                <Stack mt={5} rowGap={5}>
                            ...
                            orders && orders.map((order)=>(
                                <Stack p={is480?0:2} component={is480?"":Paper} elevation={1} rowGap={2}>
                                    ...
                                    <Stack flexDirection={'row'} rowGap={'1rem'}  justifyContent={'space-between'} flexWrap={'wrap'}>
                                        ...
                                        <Stack>
                                            <Typography>Mục: {order.item.length}</Typography>
                                        </Stack>
                                    </Stack>
                                                    ...
                                                    <Stack rowGap={1} width={'100%'}>
                                                        ...
                                                        <Stack mt={2} alignSelf={is480?"flex-start":'flex-end'} flexDirection={'row'} columnGap={2} >
                                                            <Button size='small' component={Link} to={`/product-details/${product.product._id}`} variant='outlined'>Xem sản phẩm</Button>
                                                            {
                                                                ...
                                                                <Button  size='small' variant='contained' component={Link} to={"/cart"}>Đã có trong giỏ hàng</Button>
                        ...
                        !orders.length && 
                            <Stack mt={is480?'2rem':0} mb={'7rem'} alignSelf={'center'} rowGap={2}>
                                ...
                                <Typography textAlign={'center'} alignSelf={'center'} variant='h6' >Ồ! Có vẻ bạn dạo này không đi mua sắm</Typography>
