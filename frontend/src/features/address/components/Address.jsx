    ...
    <Stack width={'100%'} p={is480?0:1}>
        ...
        <Stack p={2} position={'relative'} flexDirection={'column'} rowGap={1} component={'form'} noValidate onSubmit={handleSubmit(handleUpdateAddress)}>
            ...
            {/* if the edit is true then this update from shows*/}
            {
                ...
                (   
                    ...
                    <Stack rowGap={2}>
                        ...
                        <Stack>
                            <Typography gutterBottom>Kiểu</Typography>
                ...
                <>
                <Typography>Đường - {street}</Typography>
                <Typography>Mã bưu chính- {postalCode}</Typography>
                <Typography>Quốc giá - {country}</Typography>
                <Typography>Số điện thoại - {phoneNumber}</Typography>
                <Typography>Tình trạng - {state}</Typography>
            ...
            <Stack position={is480?"static":edit?"static":'absolute'} bottom={4} right={4} mt={is480?2:4} flexDirection={'row'} alignSelf={'flex-end'} columnGap={1}>
                ...
                {
                    edit?(<LoadingButton loading={status==='pending'} size='small' type='submit' variant='contained'>Lưu thay đổi</LoadingButton>
                    ):(<Button size='small' onClick={()=>setEdit(true)} variant='contained'>Sửa</Button>)
                }
