...
export const ProductDetails = () => {
    ...
    useEffect(()=>{
        ...
        if(cartItemAddStatus==='fulfilled'){
            ...
            toast.error('Có lỗi khi thêm sản phẩm vào giỏ hàng, vui lòng thử lại sau')
        }
    },[cartItemAddStatus])

    useEffect(()=>{
        if(wishlistItemAddStatus==='fulfilled'){
            toast.success("Thêm thành công sản phẩm vào danh sách yêu thích")
        }
        else if(wishlistItemAddStatus==='rejected'){
            toast.error("Có lỗi khi thêm sản phẩm vào danh sách yêu thích, vui lòng thử lại sau")
        }
    },[wishlistItemAddStatus])

    useEffect(()=>{
        if(wishlistItemDeleteStatus==='fulfilled'){
            toast.success("Sản phẩm đã bị xóa khỏi danh sách mong muốn")
        }
        else if(wishlistItemDeleteStatus==='rejected'){
            toast.error("Có lỗi khi xóa sản phẩm khỏi danh sách yêu thích, vui lòng thử lại sau")
        }
    },[wishlistItemDeleteStatus])

    useEffect(()=>{
        if(productFetchStatus==='rejected'){
            toast.error("Có lỗi khi tải thông tin chi tiết sản phẩm, vui lòng thử lại sau")
        }
    },[productFetchStatus])

    useEffect(()=>{
        if(reviewFetchStatus==='rejected'){
            toast.error("Có lỗi khi tải đánh giá sản phẩm, vui lòng thử lại sau")
        }
    },[reviewFetchStatus])
                        ...
                        <Stack sx={{rowGap:"1.3rem"}} width={'fit-content'}>
                            ...
                            <Stack flexDirection={"row"} columnGap={is387?".3rem":"1.5rem"} width={'100%'} >
                                ...
                                {/* add to cart */}
                                {
                                    ...
                                    <button style={{padding:"10px 15px",fontSize:"1.050rem",backgroundColor:"black",color:"white",outline:"none",border:'none',borderRadius:"8px"}} onClick={()=>navigate("/cart")}>Trong giỏ hàng</button>
                                    :<motion.button whileHover={{scale:1.050}} whileTap={{scale:1}} onClick={handleAddToCart} style={{padding:"10px 15px",fontSize:"1.050rem",backgroundColor:"black",color:"white",outline:"none",border:'none',borderRadius:"8px"}}>Thêm vào giỏ hàng</motion.button>
                                }
                        ...
                        <Stack mt={3} sx={{justifyContent:"center",alignItems:'center',border:"1px grayText solid",borderRadius:"7px"}}>
                            ...
                            <Stack p={2} flexDirection={'row'} alignItems={"center"} columnGap={'1rem'} width={'100%'} justifyContent={'flex-sart'}>
                                ...
                                <Stack>
                                    ...
                                    <Typography>Nhập mã bưu chính của bạn để biết tình trạng giao hàng</Typography>
                                </Stack>
                            </Stack>
                            <hr style={{width:"100%"}} />
